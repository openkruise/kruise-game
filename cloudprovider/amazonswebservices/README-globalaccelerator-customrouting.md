...
| `EndpointId` | one of EndpointId / EndpointIds | The registered VPC **subnet ID** the Pod runs in (the custom routing endpoint ID). Use this when the GameServerSet is pinned to a single subnet (single-AZ deployment, or one GameServerSet per AZ). |
| `EndpointIds` | one of EndpointId / EndpointIds | Comma-separated list of `subnet-id=CIDR` pairs that lets **one** GameServerSet span multiple subnets (typically multi-AZ). At reconcile time the plugin parses each Pod's IP and matches it against the CIDR of every entry; the first match wins and is used as the AGA `EndpointId` for that Pod's Allow / Deny. Example: `subnet-aaa=10.0.11.0/24,subnet-bbb=10.0.12.0/24,subnet-ccc=10.0.13.0/24`. All listed subnets MUST already be registered to the same endpoint group. Mutually exclusive with `EndpointId`. |
