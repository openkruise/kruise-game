...
func (c *ClbPlugin) OnPodUpdated(client client.Client, pod *corev1.Pod, ctx context.Context) (*corev1.Pod, cperrors.PluginError) {
	...
	log.Infof("[CLB] NetworkConfig: %+v", networkConfig)
	config := parseLbConfig(networkConfig)
	log.Infof("[CLB] Parsed clbConfig: %+v", config)
	...
	if networkManager.GetNetworkDisabled() && svc.Spec.Type == corev1.ServiceTypeLoadBalancer {
		log.Infof("[CLB] Network disabled, set svc type to ClusterIP for pod %s/%s", pod.GetNamespace(), pod.GetName())
	...
	if !networkManager.GetNetworkDisabled() && svc.Spec.Type == corev1.ServiceTypeClusterIP {
		log.Infof("[CLB] Network enabled, set svc type to LoadBalancer for pod %s/%s", pod.GetNamespace(), pod.GetName())
	...
	if util.IsAllowNotReadyContainers(networkManager.GetNetworkConfig()) {
		log.Infof("[CLB] AllowNotReadyContainers enabled for pod %s/%s", pod.GetNamespace(), pod.GetName())
...
func (c *ClbPlugin) allocate(lbIds []string, num int, nsName string, enableClbScatter ...bool) (string, []int32) {
	c.mutex.Lock()
	defer c.mutex.Unlock()
	...
	if useScatter && len(lbIds) > 0 {
		log.Infof("[CLB] scatter enabled, round robin from idx %d", c.lastScatterIdx)
		// 轮询分配
		...
		log.Infof("[CLB] scatter disabled, use default order")
		// 原有逻辑
...
func (c *ClbPlugin) consSvc(config *clbConfig, pod *corev1.Pod, client client.Client, ctx context.Context) *corev1.Service {
	...
	if exist {
		...
		lbId, ports = c.allocate(config.lbIds, len(config.targetPorts), podKey, config.enableClbScatter)
	}
