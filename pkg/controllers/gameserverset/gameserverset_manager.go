...
func computeToScaleGs(gssReserveIds, reserveIds, notExistIds []int, expectedReplicas int, pods []corev1.Pod) ([]int, []int) {
	...
	if existReplicas < expectedReplicas {
		// Add pods.
		if len(newImplicit) >= expectedReplicas-existReplicas {
			// newImplicit is sufficient
			sort.Ints(newImplicit)
			workloadManageIds = append(workloadManageIds, newImplicit[:expectedReplicas-existReplicas]...)
			newImplicit = newImplicit[expectedReplicas-existReplicas:]
		} else {
			// newImplicit is insufficient
			workloadManageIds = append(workloadManageIds, newImplicit...)
			newImplicit = []int{}
			sort.Ints(workloadManageIds)
			newValue := 0
			if len(workloadManageIds) > 0 {
				newValue = workloadManageIds[len(workloadManageIds)-1] + 1
			}
			needReplicas := expectedReplicas - len(workloadManageIds)
			toAddNum := 0
			for toAddNum < needReplicas {
				if !util.IsNumInList(newValue, newExplicit) {
					workloadManageIds = append(workloadManageIds, newValue)
					toAddNum++
				}
				newValue++
			}
