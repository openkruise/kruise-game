...
func TestOrdinalSetToIntStrSlice(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input: sets.New[int](1, 2, 3, 5, 7, 8, 9, 11),
			expected: []intstr.IntOrString{
				intstr.FromString("1-3"),
				intstr.FromInt(5),
		...
		},
		{
			...
			input: sets.New[int](5, 3, 1, 2, 4, 8, 6, 7),
			expected: []intstr.IntOrString{
				intstr.FromString("1-5"),
				intstr.FromString("6-8"),
			},
		},
		{
			name:  "non-continuous elements",
			input: sets.New[int](1, 3, 5, 7, 9),
			expected: []intstr.IntOrString{
				intstr.FromInt(1),
				intstr.FromInt(3),
				intstr.FromInt(5),
...
func TestStringToOrdinalIntSet(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			str:       "5",
			delimiter: ",",
			expected:  sets.New[int](5),
		},
		{
			name:      "multiple numbers",
			str:       "1,3,5,7",
			delimiter: ",",
			expected:  sets.New[int](1, 3, 5, 7),
		},
		{
			name:      "single range",
			str:       "1-5",
			delimiter: ",",
			expected:  sets.New[int](1, 2, 3, 4, 5),
		},
		{
			name:      "multiple ranges",
			str:       "1-3,7-9",
			delimiter: ",",
			expected:  sets.New[int](1, 2, 3, 7, 8, 9),
		},
		{
			name:      "mixed numbers and ranges",
			str:       "1-3,5,7-9,11",
			delimiter: ",",
			expected:  sets.New[int](1, 2, 3, 5, 7, 8, 9, 11),
		},
		{
			name:      "with spaces",
			str:       "1-3, 5, 7-9, 11",
