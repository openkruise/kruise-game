...
1. **NLB 分配**
   ...
   - NLB 命名规则：`{gssName}-{eipIspType}-{nlbIndex}`
   - 示例：`game-server-bgp-0`、`game-server-bgp-pro-0`

2. **EIP 分配**
   - 每个 NLB 对应多个 EIP（每个 zone 一个）
   - EIP 命名规则：`{gssName}-eip-{eipIspType}-{nlbIndex}-z{zoneIndex}`
   - 示例：`game-server-eip-bgp-0-z0`、`game-server-eip-bgp-0-z1`

3. **Service 映射**
   - 每个 Pod 对应多个 Service（每条线路一个）
   - Service 命名规则：`{podName}-{eipIspType}`
   - 示例：`game-server-0-bgp`、`game-server-0-bgp-pro`

4. **端口分配**
   - 每个 Pod 在 NLB 上分配唯一的端口范围
   - 计算公式：`port = minPort + (podIndexInNLB * portCount) + portOffset`
   - 自动跳过 `BlockPorts` 中配置的端口
