...
AutoNLBs-V3 是基于 PortAllocation 的 NLB 网络模型，面向超大规模游戏服务器场景。与 V2 按 GameServerSet 管理 NLB 资源不同，V3 通过独立的 NLB Pool Operator 将 NLB 资源池管理与 Pod 生命周期完全解耦，实现真正的资源池化、预热和跨 GSS 复用。

**核心特性：**

1. **NLBPool 资源池化**
   - NLB / ServerGroup / Listener 云资源由独立的 `NLBPool` CR 管理，与 GameServerSet 生命周期完全解耦
   - 跨 GSS 复用 — 多个 GameServerSet 可共享同一 NLBPool
   - `PortAllocation`（PA）作为 Pod 与 NLB slot 的绑定单元

2. **Slot 预热机制**
   - 所有云资源（ServerGroup + Listener）在资源池创建时预先创建完毕
   - Pod 绑定仅需对每个 ServerGroup 一次 `AddServersToServerGroup` 调用，实现亚秒级网络端点分配
   - 通过 `slotsPerNLB` 与 `minAvailableNLBs` 进行容量规划与自动扩容

3. **多线路多端口**
   - 支持多条 lane（BGP、ChinaTelecom、ChinaMobile、ChinaUnicom）同时接入
   - 每条 lane 拥有独立的 NLB 实例和 EIP
   - 每个 slot 支持多个端口（如 game / voice / http / metrics）

4. **无状态插件设计**
   - kruise-game 插件仅在 Pod 上写入 annotation；所有绑定 / 释放 / 隔离 / 解除逻辑由 NLB Pool Operator 处理
   - kruise-game 侧不持有 V3 状态，简化运维与故障恢复

- 是否支持网络隔离：是

#### 前置条件

V3 插件依赖三个阿里云 Operator（NLB Operator、EIP Operator、NLB Pool Operator）。推荐使用官方 Helm Chart [AlibabaCloud-Operator-Charts](https://github.com/chrisliu1995/AlibabaCloud-Operator-Charts) 一键安装：
...
# 从本地 aliyun CLI 配置中读取明文 AK/SK（也可从 RAM 控制台复制）
...
> Chart 已固定经过验证的镜像版本与 CRD。所需 RAM 权限：`AliyunNLBFullAccess`、`AliyunEIPFullAccess`、`AliyunVPCFullAccess`。Chart 内置校验，若传入掩码值（如 `aliyun configure get` 输出）会立即报错并提示用法。
...
> `portRange` 需要足够容纳所有 listener：`slotsPerNLB × len(ports) ≤ (max - min + 1)`。上例 `50 × 2 = 100` 个 listener，1000 个端口的范围富裕。

3. **等待资源池就绪**

```bash
kubectl get nlbpool my-pool -w
# 等待 phase=Ready 且 availableSlots = slotsPerNLB × minAvailableNLBs
```
...
- 含义：要绑定的 NLBPool CR 名称（必须与 Pod 同 namespace）
- 格式：字符串
- 是否必填：是
- 是否支持变更：否
...
```
NLBPool CR → NLB Pool Operator → 预创建 NLB + ServerGroup + Listener
                                         ↓
GameServerSet → kruise-game (V3 插件) → 在 Pod 上写入 annotation
                                         ↓
NLB Pool Operator → 为 Pod 分配 PA → AddServersToServerGroup → PA Bound
                                         ↓
kruise-game (V3 插件) → 读取 PA.spec.endpoints → 设置 GS NetworkReady
```

**绑定流程：**
1. GSS 创建 Pod → kruise-game 写入 `alibabacloud.com/nlb-pool-name` annotation
2. NLB Pool Operator 通过乐观锁原子地为 Pod 分配一个 Available 的 PA
3. NLB Pool Operator 对每个 ServerGroup 调用 `AddServersToServerGroup`（每个端口一个 SG）
4. PA 转为 Bound → kruise-game 读取 `PA.spec.endpoints` → 设置 `GS.status.networkStatus.currentNetworkState=Ready`

**释放流程：**
1. Pod 删除 → NLB Pool Operator 监听到 Pod 消失
2. PA 转为 Releasing → 对每个 SG 调用 `RemoveServersFromServerGroup`
3. PA 转回 Available，可供下次复用

**资源数量公式：**
- NLB 数 = `len(lanes) × ceil((boundSlots + slotsPerNLB × minAvailableNLBs) / slotsPerNLB)`
- EIP 数 = `NLB_count × len(zoneMaps)`
- PA 数 = `NLB_groups × slotsPerNLB`
- ServerGroup 数 = `PA数 × len(ports)`
- Listener 数 = `PA数 × len(ports) × len(lanes)`

#### 插件配置

kruise-game 侧无需额外配置。所有资源管理由 NLB Pool Operator 完成。
...
> `externalAddresses[].endPoint` 是同一 listener port 下所有 lane 的 `<NLB-DNS>/<lane-name>` 用逗号拼接的字符串。客户端可探测每条线路并选择最快的接入。

#### 注意事项

1. **NLBPool 必须在 GameServerSet 之前进入 `Ready` 状态。** 在资源池有可用 slot 之前，Pod 会一直 NotReady、PA 不会被绑定。
2. **`containerPort` 必填**：`NLBPool.spec.ports[].containerPort` 必须设置，否则后端服务器端口将回落到 listener port，流量将无法到达应用。
3. **`NETWORK_TOTAL_WAIT_TIME`**：大资源池场景下绑定耗时较长时，可在 kruise-game 设置环境变量 `NETWORK_TOTAL_WAIT_TIME=600`（秒）以延长网络就绪等待窗口。
