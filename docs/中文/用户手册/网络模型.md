...
- AlibabaCloud-Auto-NLBs 使用阿里云网络型负载均衡（NLB）CRD作为对外服务的承载实体，通过预创建NLB实例池和Service池的方式，实现游戏服的快速部署和弹性扩缩容。

- 该插件与传统的 AlibabaCloud-NLB 相比，具有以下优势：
  - 自动管理NLB实例池，根据GameServerSet的规模自动创建和管理多个NLB实例
  - 预创建Service池，提升游戏服创建速度
  - 支持多可用区部署，提高服务可用性
  - 支持多运营商线路（BGP、电信、联通等），实现更灵活的网络策略
  - 基于NLB CRD，无需直接操作云资源，更加云原生

- 是否支持网络隔离：是

- 前置条件：需要在集群中安装 [AlibabaCloud NLB Operator](https://github.com/chrisliu1995/AlibabaCloud-NLB-Operator)

#### 网络参数

EipIspType

- 含义：EIP运营商线路类型，支持配置多个运营商
- 填写格式：运营商类型用逗号分隔，例如：BGP,ChinaTelecom,ChinaUnicom
- 可选值：
  - BGP：BGP（多线）线路
  - ChinaTelecom：中国电信
  - ChinaUnicom：中国联通
  - ChinaMobile：中国移动
  - intranet：内网（需添加intranet前缀，如：intranet-BGP）
- 是否支持变更：支持
...
- 含义：NLB外部端口分配的最小值
- 填写格式：端口数字，默认为1000
- 示例：小规模测试建议使用 10000，大规模生产建议使用更大范围
- 是否支持变更：否
...
- 含义：NLB外部端口分配的最大值
- 填写格式：端口数字，默认为1499
- 示例：小规模测试建议使用 10099（100个端口），大规模生产可使用 10999 或更大
- 是否支持变更：否

BlockPorts

- 含义：需要阻塞不使用的端口列表
- 填写格式：port1,port2,port3...  例如：1010,1020,1030
- 是否支持变更：否

ZoneMaps

- 含义：NLB可用区与交换机映射关系，必须包含VPC ID
- 填写格式：vpc-id@zoneId1:vSwitchId1,zoneId2:vSwitchId2,...
- 示例：vpc-abc123@cn-hangzhou-h:vsw-xxx,cn-hangzhou-i:vsw-yyy
- 说明：至少需要配置2个可用区
- 是否支持变更：否

ReserveNlbNum

- 含义：预留的NLB实例数量，用于支持快速扩容
- 填写格式：数字，默认为1
- 是否支持变更：否
...
- 含义：Service流量策略类型
- 填写格式：Local 或 Cluster，默认为Local
- 说明：Local可以保留客户端源IP地址
- 是否支持变更：否
...
访问 nlb-xxx.cn-hangzhou.nlb.aliyuncs.com:10053 即可连接到对应的游戏服。

**架构说明**

该插件采用资源池化设计：

1. **NLB实例池**：根据GameServerSet的规模和端口配置，自动计算并创建所需数量的NLB实例
2. **Service预创建**：为每个Pod预先创建对应的Service，避免Pod启动时动态创建导致的延迟
3. **端口自动分配**：根据MinPort、MaxPort和BlockPorts配置，自动为每个游戏服分配唯一的外部端口
4. **多线路支持**：当配置多个EipIspType时，会为每个运营商创建独立的NLB实例池，实现流量分离
