...
AutoNLBs-V3 是基于 PortAllocation 的 NLB 网络模型，面向超大规模游戏服务器场景。与 V2 按 GameServerSet 管理 NLB 资源不同，V3 通过独立的 NLBPool Operator 将 NLB 资源池管理与 Pod 生命周期完全解耦，实现真正的资源池化、预热和跨 GSS 复用。

**核心特性：**

1. **NLBPool 资源池化**
   - NLB/ServerGroup/Listener 资源由独立的 NLBPool CR 管理，与 GameServerSet 生命周期完全解耦
   - 支持跨 GSS 复用 — 多个 GameServerSet 可共享同一 NLBPool
   - PortAllocation（PA）作为 Pod 与 NLB slot 的绑定单元

2. **Slot 预热机制**
   - 所有云资源（ServerGroup + Listener）在资源池创建时预先创建完毕
   - Pod 绑定仅需调用 AddServersToServerGroup（每个 SG 一次 API 调用），实现亚秒级网络端点分配
   - 可配置 `slotsPerNLB` 和 `minAvailableNLBs` 进行容量规划

3. **多线路多端口**
   - 支持多条 lane（BGP、ChinaTelecom、ChinaMobile、ChinaUnicom）同时接入
   - 每条 lane 拥有独立的 NLB 实例和 EIP
   - 支持每个 slot 多个端口（如 game/voice/http/metrics）

4. **Per-SG 注册追踪**
   - 在 `status.registeredSGs` 中追踪每个 ServerGroup 的 AddServer/RemoveServer 进度
   - 重试时跳过已完成的 SG，消除冗余 API 调用，绑定/释放效率提升约 25%

5. **无状态插件设计**
   - kruise-game 插件仅写入 annotation；所有绑定/释放逻辑由 PA Controller 处理
   - 无需管理 kruise-game 侧状态，简化运维

- 是否支持网络隔离：是

#### 前置条件

1. **部署 NLBPool Operator 和 NLB Operator**
...
# 等待 phase=Ready 且 availableSlots 等于预期数量
```

**注意事项：**
- 确保阿里云账号具备权限：`AliyunNLBFullAccess`、`AliyunEIPFullAccess`、`AliyunVPCFullAccess`
- NLBPool Operator 镜像版本：`v0.2.10-fix-releasing` 或更新
- kruise-game 镜像需包含 AutoNLBs-V3 插件：`auto-nlbs-v3-3d069e7` 或更新

#### 网络参数

NLBPoolName

- 含义：要绑定的 NLBPool CR 名称
- 格式：字符串，必须匹配同 namespace 下已存在的 NLBPool CR 名称
- 是否必填：是
- 是否支持变更：否

NLBPoolNamespace

- 含义：NLBPool CR 所在的 namespace（可选，默认为 Pod 所在 namespace）
- 格式：字符串
- 是否必填：否
- 是否支持变更：否
...
```
NLBPool CR → NLBPool Operator → 预创建 NLB + ServerGroup + Listener
                                         ↓
GameServerSet → kruise-game (V3 插件) → 在 Pod 上写入 annotation
                                         ↓
PA Controller → 为 Pod 分配 PA → AddServersToServerGroup → PA Bound
                                         ↓
kruise-game (V3 插件) → 读取 PA endpoints → 设置 GS NetworkReady
```

**绑定流程：**
1. GSS 创建 Pod → kruise-game 写入 `alibabacloud.com/nlb-pool-name` annotation
2. PA Controller 通过乐观锁为 Pod 分配一个 Available 的 PA
3. PA Controller 对每个 SG 调用 AddServersToServerGroup（4 个端口 = 4 个 SG）
4. PA 转为 Bound → kruise-game 读取 PA endpoints → GS NetworkReady

**释放流程：**
1. Pod 删除 → PA Controller 检测到 Pod 消失
2. PA 转为 Releasing → 对每个 SG 调用 RemoveServersFromServerGroup
3. PA 转为 Available → 可供下次复用

**资源数量公式：**
- NLB 数 = `len(lanes) × NLB_groups`
- EIP 数 = `len(lanes) × NLB_groups × len(zoneMaps)`
- PA 数 = `NLB_groups × slotsPerNLB`
- ServerGroup 数 = `PA数 × len(ports)`
- Listener 数 = `PA数 × len(ports) × len(lanes)`
- 自动扩容：`NLB_groups = ceil((boundSlots + slotsPerNLB × minAvailableNLBs) / slotsPerNLB)`

#### 插件配置

kruise-game 侧无需额外配置。所有资源管理由 NLBPool Operator 完成。
...
| 操作 | 速率 | 说明 |
|------|------|------|
| 预热 | ~15 PA/分钟（平均） | 受云端 API 限速影响 |
| 绑定（100 Pod） | ~87s | ~54 PA/分钟 |
| 绑定（200 Pod） | ~222s | 含节点扩容时间 |
| 释放（200 Pod） | ~431s | ~28 PA/分钟 |

#### 注意事项

1. **NLBPool CR 必须在 GameServerSet 之前创建** — Pod 会一直 NotReady 直到资源池有可用 slot
2. **containerPort 必填** — NLBPool `spec.ports[].containerPort` 必须设置，否则后端服务器端口默认为 listenerPort（大多数场景下不正确）
3. **V6 架构** — ServerGroup 和 Listener 是云端资源（无 K8s CR），对账需使用阿里云 CLI 或控制台
4. **NETWORK_TOTAL_WAIT_TIME** — 大规模场景（绑定可能需要数分钟）时在 kruise-game 中设置为 600（秒）
