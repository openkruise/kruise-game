...
OpenKruiseGame（OKG）是像Deployment一样的Kubernetes的工作负载，相比Kubernetes内置的工作负载，例如：Deployment、StatefulSet等而言，OpenKruiseGame（OKG）提供了热更新、原地升级、定向管理等常用的游戏服管理功能，是完全面向游戏服场景而设计的Kubernetes工作负载。
